...
function close_cancel(no){
    $('#grid').load('./modules/hris/employee_vl.php?emp_no=100'+ no);
}
    ...
    w2confirm('Proceed to update?', function (btn) {
        ...
        $.ajax({
            ...
            if (jObject.status === "success") {
                $('#grid').load('./modules/hris/employee_vl.php?emp_no=100'+ $('button#save').data('recid'));
            } else {
