    ...
    $cfn->log_activity("EMPLOYEE SHIFT SCHEDULE");
    global $db, $hris;
    ...
    function reset_shift(emp_no) {
        $('#grid').load('./modules/hris/master_schedule.php?emp_no='+ emp_no +'&cmd=edit&set=' + $("select#shiftset").val());
    }
                ...
                if (btn === "Yes") {
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            $('#grid').load('./modules/hris/master_schedule.php?emp_no='+ $('button#save').data('recid') + '&cmd=edit');
                            } else {
                            w2alert(jObject.message);
                            }
                        },
